...
using (var scope = app.Services.CreateScope())
{
    ...
    while (retryCount < maxRetries)
    {
        try
        {
            ...
            logger.LogInformation($"Attempting database migration (attempt {retryCount + 1}/{maxRetries})...");
        ...
        catch (Exception ex)
        {
            ...
            {
                logger.LogError(ex, "Failed to migrate database after {retryCount} attempts.", maxRetries);
                throw;
            }
            logger.LogWarning($"Database migration attempt {retryCount} failed. Retrying in {delaySeconds} seconds...");
